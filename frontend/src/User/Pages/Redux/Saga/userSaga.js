...
function* loginUserSaga(action) {
    try {
        const response = yield call(axios.post, `${process.env.REACT_APP_BASE_URL}/api/admin/user/login`, {
            ...action.payload,
            role: 'user',
        });
...
function* fetchUserProfileAndHistory() {
    try {
        ...
        const profileResponse = yield call(axios.get, `${process.env.REACT_APP_BASE_URL}/api/user/profile`, config);
...
function* fetchUserDashboard() {
    try {
        ...
        const [booksRes, purchasedRes, paymentsRes] = yield all([
            call(axios.get, `${process.env.REACT_APP_BASE_URL}/api/book/available`),
            call(axios.get, `${process.env.REACT_APP_BASE_URL}/api/mybooks/count`, config),
            call(axios.get, `${process.env.REACT_APP_BASE_URL}/api/payment/user-summary`, config),
        ]);
...
function* handleChangePassword(action) {
    try {
        ...
        const response = yield call(() =>
            axios.put(
                `${process.env.REACT_APP_BASE_URL}/api/user/change-password`,
...
function* registerUserSaga(action) {
    try {
        ...
        yield call(() =>
            axios.post(`${process.env.REACT_APP_BASE_URL}/api/auth/register`, formData, {
