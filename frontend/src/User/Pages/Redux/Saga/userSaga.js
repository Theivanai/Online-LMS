...
function* loginUserSaga(action) {
    try {
        const response = yield call(axios.post, `${process.env.REACT_APP_BACKEND_URL}/api/admin/user/login`, {
            ...action.payload,
            role: 'user',
        });
...
function* fetchUserProfileAndHistory() {
    try {
        ...
        const profileResponse = yield call(axios.get, `${process.env.REACT_APP_BACKEND_URL}/api/users/profile`, config);
...
function* fetchUserDashboard() {
    try {
        ...
        const [booksRes, purchasedRes, paymentsRes] = yield all([
            call(axios.get, `${process.env.REACT_APP_BACKEND_URL}/api/book/available`),
            call(axios.get, `${process.env.REACT_APP_BACKEND_URL}/api/mybooks/count`, config),
            call(axios.get, `${process.env.REACT_APP_BACKEND_URL}/api/payment/user-summary`, config),
        ]);
...
function* registerUserSaga(action) {
    try {
        ...
        yield call(() =>
            axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/user/register`, formData, {
