...
function* checkBookStatusSaga(action) {
    try {
        ...
        const res = yield call(() =>
            axios.get(`${process.env.REACT_APP_BASE_URL}/api/payment/book-status/${bookId}`, {
...
function* fakePaymentSaga(action) {
    try {
        ...
        yield call(
            axios.post,
            `${process.env.REACT_APP_BASE_URL}/api/payment/verify`,
        ...
        yield call(
            axios.put,
            `${process.env.REACT_APP_BASE_URL}/api/payment/reduce-stock/${bookId}`,
            {},
            { headers: { Authorization: `Bearer ${token}` } }
        );
