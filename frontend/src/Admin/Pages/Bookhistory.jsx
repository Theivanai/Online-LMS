...
const Bookhistory = () => {
  ...
  return (
    <Container className={styles.container}>
      ...
      <InputGroup className={`${styles.searchBox} mb-3`}>
        <Form.Control
          type="text"
          placeholder="Search by user name or email"
          value={searchTerm}
      ...
      <div className={styles.tableWrapper}>
        <Table bordered hover striped className={`align-middle text-center ${styles.table}`}>
          <thead>
            <tr>
              <th>IMAGE</th>
              <th>BOOK ID</th>
            ...
            {filteredHistory.length === 0 ? (
              ...
              filteredHistory.map((record) => (
                <tr key={record._id}>
                  ...
                  <td>{record.startDate ? new Date(record.startDate).toLocaleString('en-IN') : 'Not available'}</td>
                  <td>{record.endDate ? new Date(record.endDate).toLocaleString('en-IN') : 'Not available'}</td>
                  <td className={styles.amountPaid}>₹{record.amountPaid?.toFixed(2) || '0.00'}</td>
                </tr>
              ))
            )}
          </tbody>
